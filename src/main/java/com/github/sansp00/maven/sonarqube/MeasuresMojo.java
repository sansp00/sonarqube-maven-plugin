...
public class MeasuresMojo extends AbstractProjectSonarQubeMojo {
	...
	public void execute() throws MojoExecutionException {
		...
		try {
			...
			Optional<MeasureComponentResponse> measureResponse = client.measures().component(getKey(), metricKeys,
					ADDITIONNAL_FIELDS);
	...
	void outputComponent(final Component component, final List<Period> periods) {
		...
		Rating leakReliabilityRating = Rating
				.valueOfDouble(NumberUtils.toDouble(measures.get("new_reliability_rating")));
		Rating leakSecurityRating = Rating.valueOfDouble(NumberUtils.toDouble(measures.get("new_security_rating")));
		Rating leakMaintainabilityRating = Rating
				.valueOfDouble(NumberUtils.toDouble(measures.get("new_maintainability_rating")));
		...
		info(String.format("|  * vulnerabilities [rating: '%s', amount: '%s']", securityRating,
		...
		info(String.format("|  * vulnerabilities [rating: '%s', amount: '%s']", leakSecurityRating,
				measures.get("new_vulnerabilities")));
		info(String.format("|  * code Smells [rating: '%s', amount: '%s']", leakMaintainabilityRating,
				measures.get("new_code_smells")));
